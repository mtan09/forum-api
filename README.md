...
| Daily Brief | `npm run daily-briefs` | Dedicated 15-minute Railway cron; generates timezone-aware opted-in delivery and prunes seven-day snapshots |
...
- **Private accounts** — follows have pending/accepted states. Unapproved visitors see basic profile identity and spectrum but not collected Posts/Comments/Upvoted/Saved history; individually encountered content remains eligible for feeds, search, and threads. A private recipient accepts DMs only from an account they follow; the server enforces this before moderation or insertion, while existing thread history remains readable. Blocking removes follow relationships.
- **Notifications + Daily Brief** — Push and Email preferences exist per replies/upvotes/DMs/follows. Email delivery is globally opt-in and requires a verified address; replies and DMs send immediately and upvotes coalesce per post. The optional 7:00 AM local Daily Brief persists a seven-day, first-party snapshot of hot stories, tailored posts, Floor rooms, and grouped personal activity; email and push are independently opt-in. Successful Expo ticket IDs are persisted, their APNs/FCM receipts are checked after the recommended delay, and dead device tokens are removed.
...
- **Deletion and feedback** — account deletion immediately removes structured feedback along with the profile and other account data, then transactionally enqueues public/private R2 cleanup with retries and a 24-hour alert. Authors can also delete an individual post or comment: ownership is enforced by the API, post threads/comment reply subtrees cascade, cached comment counts are corrected transactionally, shared DMs receive a tombstone, and an exact-object cleanup job removes only that post's uploaded media. While an account exists, structured feedback stores device/build context and optional screenshots in a separate private bucket; only admins receive short-lived signed URLs.
- **Reliable ingestion** — a heartbeat keeps the PgBouncer-safe Postgres advisory lock alive during network-heavy runs, abandoned runs are reconciled after the lock is reacquired, database and per-source failures retry independently, clustering remains in the successful flow, and `ingest_runs` records totals, failures, duration, and freshness.
...
| `GET /users?ids=` · `GET /users/:id` · `GET /users/:id/spectrum` | ✅ | Public profiles, follow/block state, DM eligibility, counts, and computed spectrum |
...
| `GET /briefs/today`, `GET /briefs`, `GET /briefs/:date`, `POST /briefs/:id/seen` | ✅ | Timezone-aware Daily Brief generation, archive, detail, and cross-device seen state |
...
| `DELETE /comments/:id` | ✅ | Permanently delete the caller's own comment subtree and reconcile parent counts |
...
| `GET /messages` · `/unread-count` · `/with/:userId` | ✅ | DM inbox, unread total, and a read-marking conversation thread with typed post/article preview metadata; admin-hidden messages are omitted |
| `POST /messages/with/:userId` `{content?,shared_kind?,shared_id?}` | ✅ | Send a block-aware, rate-limited text message or validated in-app content share |
...
- Daily Brief: `DAILY_BRIEF_ENABLED=yes` on the API and dedicated `forum-daily-brief` cron; the cron runs `npm run daily-briefs` every 15 minutes with restart policy `Never`.
